...
import { Camera, MapPin, Mail, Compass } from "lucide-react";
...
export function ProfileHero({
    name, email, bio, profileImage, location, locationPreference, socialLinks, completeness, onImageChange
}: ProfileHeroProps) {
    ...
    const socialItems = [
        { url: socialLinks?.linkedin, icon: FaLinkedin, label: "LinkedIn", color: "hover:text-blue-400 hover:bg-blue-500/10" },
        { url: socialLinks?.github, icon: FaGithub, label: "GitHub", color: "hover:text-white hover:bg-white/10" },
        { url: socialLinks?.portfolio, icon: FaGlobe, label: "Portfolio", color: "hover:text-emerald-400 hover:bg-emerald-500/10" },
        { url: socialLinks?.twitter, icon: FaXTwitter, label: "Twitter", color: "hover:text-sky-400 hover:bg-sky-500/10" },
    ].filter(s => s.url);

    const completenessColor =
        completeness >= 80 ? "from-emerald-500 to-emerald-400" :
        completeness >= 50 ? "from-amber-500 to-amber-400" :
    ...
    return (
        <div className="relative overflow-hidden rounded-2xl border border-border/40 bg-card">
            ...
            <div className="absolute top-0 right-0 w-80 h-80 bg-primary/8 rounded-full blur-3xl -translate-y-1/2 translate-x-1/4 animate-pulse" style={{ animationDuration: "4s" }} />
            <div className="absolute bottom-0 left-0 w-56 h-56 bg-secondary/8 rounded-full blur-3xl translate-y-1/2 -translate-x-1/4 animate-pulse" style={{ animationDuration: "6s" }} />
            ...
            <div className="relative z-10 p-5 sm:p-6 md:p-8">
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 sm:gap-6">
                    ...
                    <div className="relative group shrink-0">
                        <div className="absolute -inset-1.5 bg-gradient-to-br from-primary via-secondary to-primary rounded-full opacity-40 blur-md group-hover:opacity-70 transition-all duration-500" />
                        <Avatar className="h-24 w-24 sm:h-28 sm:w-28 md:h-32 md:w-32 relative border-3 border-background shadow-xl shadow-primary/10">
                            <AvatarImage src={profileImage || "/placeholder-avatar.jpg"} alt={name} className="object-cover" />
                            <AvatarFallback className="text-2xl md:text-3xl font-bold bg-gradient-to-br from-primary/20 to-secondary/20 text-primary">
                ...
                <div className="mt-6 pt-5 border-t border-border/30">
                    <div className="flex items-center justify-between mb-2.5">
                        ...
                        <span className={`text-sm font-bold tabular-nums ${completeness >= 80 ? "text-emerald-400" : completeness >= 50 ? "text-amber-400" : "text-primary"}`}>
                            {completeness}%
                        </span>
                    </div>
                    <div className="w-full h-2.5 bg-muted/30 rounded-full overflow-hidden backdrop-blur-sm">
                        <div
                            className={`h-full rounded-full bg-gradient-to-r ${completenessColor} transition-all duration-1000 ease-out shadow-sm`}
